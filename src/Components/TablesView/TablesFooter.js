...
function TablesFooter({
	...
	useEffect(() => {
		if (activeButton === "Edit") {
			...
			setInFuse((fused) => {
				return { type: "None", fusedList: [], sepList: [...fused.sepList] };
			});
			setInEdit(false);
		}
	...
	const checkTablePos = (table1, table2) => {
		const diffTop = Math.abs(table2.top - table1.top);
		const diffLeft = Math.abs(table2.left - table1.left);
		if (diffTop > diffLeft) {
			return true;
			//fusion vers le bas
		}
	...
	const onFuseAdd = () => {
		if (inFuse.fusedList.length > 1) {
			setBoard((prevBoard) => {
				...
				const table1Occupied = table1.time !== "00:00";
				const table2Occupied = table2.time !== "00:00";
				return prevBoard.reduce((newBoard, table) => {
					const table1Check =
						table.left === table1.left && table.top === table1.top;
					const table2Check =
						table.left === table2.left && table.top === table2.top;
					if (table1Check) {
						...
						const updatedTable = {
							...
							fused: [
								...(table.fused || []),
								JSON.parse(JSON.stringify(table2)),
							],
						};
						if (table2Occupied && !table1Occupied) {
							updatedTable.time = table2.time;
							updatedTable.orderId = table2.orderId;
	...
	const onSepAdd = () => {
		setBoard((prevBoard) => {
			...
			const updatedBoard = prevBoard.map((table) => {
				const isFused =
					table.left === fusedTable.left && table.top === fusedTable.top;
			...
			return lastFusedToRestore
				? [...updatedBoard, lastFusedToRestore]
				: updatedBoard;
		});
		setInFuse((prevFuse) => ({
			...prevFuse,
	...
	return (
		<div className="row-span-1 bg-kitchen-blue shadow-button grid grid-flow-col grid-cols-12">
			...
			{activeButton === "Fuse" && (
				<div className="col-span-4 grid grid-flow-col grid-cols-2 justify-center justify-items-center">
					<div
						onClick={() => onFuseAdd()}
						id="fuse-add"
						className="col-span-1 flex items-center justify-center w-3/4 h-2/4 font-bold text-sm md:text-lg lg:text-3xl text-white bg-kitchen-button-green self-center rounded-full"
					>
						Fusionner
					</div>
					<div
						onClick={() => onDel()}
						id="fuse-del"
						className="col-span-1 flex items-center justify-center w-3/4 h-2/4 font-bold text-sm md:text-lg lg:text-3xl text-white bg-kitchen-button-red self-center rounded-full"
					>
						Annuler
					</div>
				</div>
			...
			{activeButton === "Sep" && (
				<div className="col-span-9 grid grid-flow-col grid-cols-9 justify-center justify-items-center">
					<div
						onClick={() => onSepAdd()}
						id="sep-add"
						className="col-span-2 flex items-center justify-center w-3/4 h-2/4 font-bold text-sm md:text-lg lg:text-3xl text-white bg-kitchen-button-green self-center rounded-full"
					>
						Séparer
					</div>
					<div
						onClick={() => onDel()}
						id="sep-del"
						className="col-span-2 flex items-center justify-center w-3/4 h-2/4 font-bold text-sm md:text-lg lg:text-3xl text-white bg-kitchen-button-red self-center rounded-full"
					>
						Annuler
					</div>
			...
			{activeButton === "Edit" && <Tables />}
			{activeButton === "None" && <div id="blank" className="col-span-9" />}
			{activeButton === "Fuse" && <div id="blank" className="col-span-5" />}
			<FooterButton
				url="edit.png"
				type="Edit"
				activeButton={activeButton}
