...
export default function TablesView({
	...
	useEffect(() => {
		if (inEdit === false) {
			...
			if (isFirstRender === false) setDataToBeSaved(true);
		}
	...
	const createConfigBoard = (board) => {
		const configBoard = board.map((table) => {
			let obj = {
				x: table.left,
				y: table.top,
	...
	const hasInvalidPlates = (tables) => {
		for (const table of tables) {
			if (table.plates <= 0) {
				return true;
			}
			if (table.fused && table.fused.length > 0) {
				if (hasInvalidPlates(table.fused)) {
					return true;
				}
			}
		}
	...
	const saveTable = () => {
		if (hasInvalidPlates(board)) {
			return;
		}

		const configBoard = createConfigBoard(board);
