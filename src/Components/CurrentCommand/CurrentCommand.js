...
function Food({ name, price, quantity }) {
	return (
		<div className="w-full flex flex-row justify-between">
			<div className="flex text-24px text-white font-light">
				{quantity}x {name}
			</div>
			<div className="flex text-24px justify-end text-white font-light">
				{price * quantity}€
			</div>
		</div>
	);
...
const Sup = ({ text }) => (
	<div className="w-full flex flex-row gap-2 pl-3 items-center">
		<div
			className={`flex text-20px text-white font-light w-full ${text.suppPrice ? "justify-between" : ""}`}
		>
			<div>
				{text.type === "ADD"
					? `Supplement ${text.ingredient}`
					: text.type === "ALE"
						? `Allergie ${text.ingredient}`
						: text.type === "DEL"
							? `Sans ${text.ingredient}`
							: ``}
			</div>
			<div>
...
const Stop = () => (
	<div className="w-full h-5 gap-2 flex flex-row items-center">
		...
		<div className="w-1/3 h-full flex items-center justify-center font-bold text-kitchen-yellow text-4xl">
			STOP
		</div>
		<div className="w-1/3 h-2/3 bg-kitchen-yellow"></div>
	</div>
	...
	return (
		<div className="w-full h-10 pl-1 mt-2 flex flex-row justify-center self-center items-center bg-white rounded-lg">
			<button
				...
				className="text-2xl font-semibold text-white w-1/5 h-5/6 mr-1 bg-kitchen-button-darkgreen rounded-l-md"
			>
				+
			</button>
			<button
				onClick={() => delOrderQuantity()}
				className="text-2xl font-semibold text-white w-1/5 h-5/6 bg-kitchen-button-red"
			>
				-
			</button>
			<button
				onClick={() => modifyFoodOrder()}
				className="text-2xl font-semibold text-kitchen-blue w-3/5 h-5/6 bg-white"
			>
				Modifier
			</button>
		</div>
...
function Order({ order, border, config, setOrders }) {
	...
	return (
		<div
			className={`w-full h-auto p-2 ${border ? "border-t border-t-kitchen-yellow" : ""}`}
		>
			<div onClick={() => handleOrderEdit()}>
				...
				{order.mods_ingredients &&
					order.mods_ingredients.map((sup, index) => (
...
const Content = ({ orders, stop, config, setOrders }) => (
	<div
		className={
			!config.payement
				? "w-full max-h-none sm:max-h-[60vh] flex flex-col"
				: "w-full max-h-none sm:max-h-[60vh] flex flex-col border-b-4 border-kitchen-yellow box-border border-solid"
		}
	>
		{orders.food.map((order, index) => {
			...
			if (config.payement && order.stop === true) return null;
...
function Footer({
	...
	async function sendFirstOrder() {
		...
		const promises = orders.food.map(async (order) => {
			...
			for (let i = 0; i !== number; i++) {
				...
				const newObj = Object.keys(cleanOrder).reduce((acc, key) => {
					if (key !== "name") {
						if (key === "price") {
		...
		if (orders.orderId !== null) {
			await fetch(
				`${process.env.REACT_APP_BACKEND_URL}:${process.env.REACT_APP_BACKEND_PORT}/api/${localStorage.getItem("restaurantID")}/orders/${orders.orderId}`,
				{
					method: "PUT",
					headers: {
			...
			await fetch(
				`${process.env.REACT_APP_BACKEND_URL}:${process.env.REACT_APP_BACKEND_PORT}/api/${localStorage.getItem("restaurantID")}/orders${orders.tableId ? `?idTable=${orders.tableId}` : ""}`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
	...
	async function sendOtherOrder() {
		...
		if (index !== -1) {
			fetch(
				`${process.env.REACT_APP_BACKEND_URL}:${process.env.REACT_APP_BACKEND_PORT}/api/${localStorage.getItem("restaurantID")}/orders/next/${config.id_order}`,
				{
					method: "PUT",
	...
	if (!config.payement) {
		return (
			<div className="w-full h-current-cmd-footer border-t border-kitchen-yellow flex flex-row gap-px bg-kitchen-yellow">
				<div
					className="w-1/2 h-full bg-kitchen-blue flex items-center justify-center text-white font-bold text-testpx text-center cursor-pointer"
					onClick={() => addStop()}
				>
					STOP
				</div>
					...
					{priceLess < 0 ? (
						<div className="flex flex-row justify-between w-full">
							...
							<div className="text-white font-bold text-24px">
								{(priceLess * -1).toFixed(2).toString()}€
							</div>
						</div>
					) : (
						<div className="flex flex-row justify-between w-full">
	...
	return (
		<div className="h-full w-full bg-kitchen-blue float-right flex flex-col justify-between overflow-hidden">
			<div
				className={
					!config.payement
						? "w-full max-h-[85%] float-right px-2 gap-3 flex flex-col"
						: "w-full max-h-[80%] float-right px-2 gap-3 flex flex-col"
				}
			>
