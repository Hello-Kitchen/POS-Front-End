			...
			<div className="w-full h-full bg-kitchen-yellow flex flex-row gap-0.5">
				...
				<div className="bg-kitchen-blue h-full w-2/10 lg:w-1/10 flex">
					<div
						...
						className="w -1/2 lg:w-full h-full flex cursor-pointer"
						onClick={() => {
					...
					<div
						className="w-1/2 h-full bg-kitchen-blue flex cursor-pointer lg:hidden items-center justify-center"
						onClick={() => {
							setDrawerOpen(true);
						}}
					>
...
export function FooterMainButton({
	price,
	config,
	setConfig,
	priceLess,
	...
	const handlePayement = async () => {
		const body = {
			value: payDetail,
			user: Number(JSON.parse(localStorage.getItem("userInfo")).id),
			discount: 0,
		};
		await fetch(
			`${process.env.REACT_APP_BACKEND_URL}:${process.env.REACT_APP_BACKEND_PORT}/api/${localStorage.getItem("restaurantID")}/orders/payment/${config.id_order}`,
			{
				method: "PUT",
		...
		<div className="w-full h-full">
			{priceLess > 0 || !config.payement ? (
				<div className="w-full h-full bg-kitchen-yellow flex justify-center items-center p-3 shadow-[inset_0_10px_50px_-20px_rgba(0,0,0,0.7)]">
					<div
						className="w-full h-full flex justify-center items-center truncate text-4xl font-bold text-kitchen-blue cursor-pointer"
						onClick={() => {
							setConfig((prevConfig) => ({
								...prevConfig,
								payement: !prevConfig.payement,
							}));
							navigate(!config.payement ? "/dashboard/pay" : "/dashboard");
						}}
					>
						{!config.payement
							? `Encaisser ${Number(price).toFixed(2).toString()}€`
							: "Retour"}
					</div>
				</div>
			) : (
